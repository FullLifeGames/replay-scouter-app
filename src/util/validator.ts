  ...
  for (const rawSearchQuery of rawSearchQueries) {
    ...
    if (team.pokemon) {
      valid ||= team.pokemon.some(
        (pokemon) => pokemon.name?.toLowerCase().includes(searchQuery),
      );
      valid ||= team.pokemon.some(
        (pokemon) =>
      ...
      valid ||= team.pokemon.some(
        (pokemon) => pokemon.ability?.toLowerCase().includes(searchQuery),
      );
      valid ||= team.pokemon.some(
        (pokemon) => pokemon.item?.toLowerCase().includes(searchQuery),
      );
      valid ||= team.pokemon.some(
        (pokemon) =>
      ...
      valid ||= team.pokemon.some(
        (pokemon) => pokemon.teraType?.toLowerCase().includes(searchQuery),
      );
      valid ||= team.pokemon.some(
        (pokemon) => pokemon.formName?.toLowerCase().includes(searchQuery),
      );
    }
    if (team.format) {
    ...
    if (team.replays) {
      valid ||= team.replays.some(
        (replay) =>
          replay.playerInfo?.playerName?.toLowerCase().includes(searchQuery),
      );
      valid ||= team.replays.some(
        (replay) => replay.id?.toLowerCase().includes(searchQuery),
      );
    }
